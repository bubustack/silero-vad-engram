...
IMG ?= ghcr.io/bubustack/silero-vad-engram:dev
...
ONNXRUNTIME_PKG ?= libonnxruntime
CGO_CFLAGS += $(shell $(PKG_CONFIG) --cflags $(ONNXRUNTIME_PKG) 2>/dev/null)
CGO_LDFLAGS += $(shell $(PKG_CONFIG) --libs $(ONNXRUNTIME_PKG) 2>/dev/null)
export CGO_CFLAGS
export CGO_LDFLAGS
